...
MatmulFn make_tiled_gpu_fn()
{
  return [] (const Matrix& a, const Matrix& b, Matrix& c) {
    matmul_tiled_gpu(a, b, c);
  };
}
...
int main()
{
  ...
  num_fails += run_random_tests(make_tiled_gpu_fn(), gen, small, 10,
                                "Trial 1 (random small matrices):");

  num_fails += run_random_tests(make_tiled_gpu_fn(), gen, medium, 10,
                                "Trial 2 (random medium matrices):");

  num_fails += run_random_tests(make_tiled_gpu_fn(), gen, large, 10,
